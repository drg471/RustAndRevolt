...
data class Player(
    ...
    val name: String,
    val remainingHealPotions: Int,
)

data class Controls(
    val showSeqText: String,
    val enableBtnHeal: Boolean,
    val enableBtnStrongAttack: Boolean,
    val enableBtnVeryStrongAttack: Boolean,
    val enableBtnSpecialAttack: Boolean,
    val enableBtnEndGame: Boolean,
)

sealed interface CombatState {
    object Loading : CombatState
    data class Playing(val player: Player, val enemy: Character, val controls: Controls) : CombatState
    data class StopPlaying(val player: Player, val enemy: Character, val controls: Controls) : CombatState
}
...
class CombatViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow<CombatState>(CombatState.Loading)
    val state: StateFlow<CombatState> = _state

    init {
        _state.value = CombatState.Loading

        viewModelScope.launch {
            _state.emit((_state.value as CombatState.Playing).copy(
                player = (_state.value as CombatState.Playing).player.copy(life =  100f)
            ))
